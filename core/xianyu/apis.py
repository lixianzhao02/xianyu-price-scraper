...
def build_initial_cookies() -> requests.Session:
    ...
    s.verify = False  # 某些网络环境 SSL 证书有问题
...
def qrcode_login(poll_interval: float = 3.0, timeout: float = 120.0,
    ...
    qr_logger.info('=' * 60)
    qr_logger.info('请用闲鱼 APP 扫描下方二维码登录')
    qr_logger.info('打开闲鱼 APP → 左上角扫一扫')
    qr_logger.info('二维码 URL: %s', qr_url)
    qr_logger.info('=' * 60)

    print(f'\n{"=" * 60}')
    print(f'  请用闲鱼 APP 扫描下方二维码登录')
    print(f'  打开闲鱼 APP → 左上角扫一扫')
    print(f'  二维码 URL: {qr_url}')
    print(f'  如果二维码太小无法扫描，请复制上面的 URL 到浏览器打开')
    print(f'{"=" * 60}\n')
    ...
    if show_qrcode:
        try:
            ...
            import sys
            qr = qr_lib.QRCode(border=2, box_size=2)
            qr.add_data(qr_url)
            qr.make()
...
class XianyuApis:
    def __init__(self, cookies, device_id):
        ...
        # 统一用 .goofish.com domain 设置 cookie，避免多 domain 冲突
    ...
    def get_token(self):
        ...
        token = self._get_cookie('_m_h5_tk').split('_')[0]
        sign = generate_sign(params['t'], token, data_val)
        params['sign'] = sign
        response = self.session.post(self.login_url, params=params, headers=headers, data=data)
        # 更新 session cookie：清除所有同名的旧 cookie，再设置新值
        for response_cookie_key, response_cookie_value in response.cookies.get_dict().items():
            # 清除所有同名的旧 cookie（不管 domain）
            to_remove = []
            for key in self.session.cookies:
                if key.name == response_cookie_key:
                    to_remove.append(key)
            for key in to_remove:
                self.session.cookies.clear(domain=key.domain, path=key.path, name=key.name)
            # 设置新 cookie（用 .goofish.com domain）
            self.session.cookies.set(response_cookie_key, response_cookie_value,
    ...
    def search(self, keyword: str, page: int = 1, page_size: int = 30) -> dict:
        """
        搜索闲鱼商品
        :param keyword: 搜索关键词
        :param page: 页码（从1开始）
